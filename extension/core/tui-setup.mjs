...
export const SETUP_OPTIONS = Object.freeze({
  ...
  toolsInherit: "Inherit every active Pi tool (recommended)",
  toolsRestrict: "Use an explicit Cascade tool allowlist",
...
export function persistCascadeConfig({ cwd, scope, config }) {
  if (scope === "session") return { path: undefined, scope, config: deepClone(config) };
...
async function chooseToolPolicy(ctx, role, current) {
  const selected = await ctx.ui.select(`Cascade setup · ${role} tools`, [
    SETUP_OPTIONS.toolsInherit,
    SETUP_OPTIONS.toolsRestrict
  ]);
  if (!selected) return undefined;
  if (selected === SETUP_OPTIONS.toolsInherit) return { restrictTools: false };
  const existing = Array.isArray(current?.tools) ? current.tools.join(",") : "read,grep,find,ls,bash,edit,write";
  const value = await ctx.ui.input(`Cascade setup · ${role} tool allowlist`, existing);
  if (value === undefined) return undefined;
  const tools = [...new Set(value.split(",").map((entry) => entry.trim()).filter(Boolean))];
  if (!tools.length) {
    ctx.ui.notify("A restrictive tool policy needs at least one tool.", "error");
    return chooseToolPolicy(ctx, role, current);
  }
  return { restrictTools: true, tools };
}

async function choosePositiveNumber(ctx, title, current, { integer = false } = {}) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0 || (integer && !Number.isInteger(parsed))) {
    ...
    return choosePositiveNumber(ctx, title, current, { integer });
  }
  return parsed;
}
...
function setupSummary(config, scope) {
  ...
  return [
    ...
    `Mode: ${config.mode}`,
    `Worker: ${worker}`,
    `Worker tools: ${config.worker.restrictTools ? "restricted" : "inherit Pi"}`,
    `Expert: ${expert}`,
...
export async function runCascadeSetup({ ctx, config }) {
  ...
  if (workerMode === SETUP_OPTIONS.workerNative) {
    ...
    const tools = await chooseToolPolicy(ctx, "Worker", selectedWorker.profile);
    if (!tools) return { cancelled: true };
    next.worker = { ...selectedWorker.profile, ...thinking, ...tools };
  ...
  if (next.mode === "dual") {
    ...
    if (!thinking) return { cancelled: true };
    const tools = await chooseToolPolicy(ctx, "Expert", selectedExpert.profile);
    if (!tools) return { cancelled: true };
    next.expert = { ...selectedExpert.profile, ...thinking, ...tools };
  ...
  const contributorSelected = [next.worker, next.mode === "dual" ? next.expert : undefined]
    .filter(Boolean)
    .some((profile) => profile.selectionMode === "configured" && isContributorModel(profile, next.privacy.contributorPattern));
  ...
  if (budgetMode === SETUP_OPTIONS.editBudgets) {
    const calls = await choosePositiveNumber(ctx, "Maximum expert calls per session", next.budgets.maxExpertCalls, { integer: true });
...
export async function runRoleModelPicker({ ctx, config, role }) {
  ...
  if (!thinking) return { cancelled: true };
  const tools = await chooseToolPolicy(ctx, role === "worker" ? "Worker" : "Expert", selected.profile);
  if (!tools) return { cancelled: true };
  next[role] = { ...selected.profile, ...thinking, ...tools };
  const scope = await chooseScope(ctx);
  if (!scope) return { cancelled: true };
  const persisted = persistCascadeConfig({ cwd: ctx.cwd, scope, config: next });
  return {
