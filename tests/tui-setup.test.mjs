  ...
  const ctx = {
    ...
    ui: {
      async select(title, options) {
        ...
        if (title.includes("Expert tools")) return SETUP_OPTIONS.toolsInherit;
...
test("TUI setup configures dual roles and writes project settings without secrets", async () => {
  ...
  assert.equal(result.config.expert.restrictTools, false);
  ...
  const savedPath = join(cwd, ".cascade", "config.json");
  assert.ok(existsSync(savedPath));
  const saved = readFileSync(savedPath, "utf8");
  assert.equal(/sk-(?:or|ant|proj|live)-[A-Za-z0-9_-]{12,}/i.test(saved), false);
  assert.match(saved, /\$MODEL_API_KEY/);
