...
import { getPiGlobalCompaction, writePiGlobalCompaction } from "../extension/core/pi-settings.mjs";
...
Usage:
  cascade [cascade options] [--] [pi options and prompt]
  cascade init [--global] [--force]
  cascade doctor [--cascade-config FILE] [--approve]
  cascade config show|path|example
  cascade harness list|candidates|evaluate|replay|promote|rollback ...
...
Examples:
  cascade --approve --worker meta-model-api/muse-spark-1.2-contributor \\
    --expert openrouter/anthropic/claude-opus-5 "Fix the failing tests"

  cascade --single --worker openrouter/openai/gpt-5.6-codex "Refactor this module"
`;
}
...
function parseCascadeOptions(argv) {
  ...
  while (index < argv.length) {
    ...
    if (pair("--cascade-config", "CASCADE_CONFIG")) { explicitConfig = env.CASCADE_CONFIG; continue; }
    if (pair("--cascade-mode", "CASCADE_MODE")) continue;
    if (pair("--worker", "CASCADE_WORKER")) continue;
    if (pair("--expert", "CASCADE_EXPERT")) continue;
    if (pair("--worker-thinking", "CASCADE_WORKER_THINKING")) continue;
    if (pair("--expert-thinking", "CASCADE_EXPERT_THINKING")) continue;
    if (pair("--worker-tools", "CASCADE_WORKER_TOOLS")) continue;
    if (pair("--expert-tools", "CASCADE_EXPERT_TOOLS")) continue;
    if (pair("--worker-instructions", "CASCADE_WORKER_INSTRUCTIONS")) continue;
    if (pair("--expert-instructions", "CASCADE_EXPERT_INSTRUCTIONS")) continue;
    if (pair("--expert-timeout-ms", "CASCADE_EXPERT_TIMEOUT_MS")) continue;
    if (pair("--expert-max-output-characters", "CASCADE_EXPERT_MAX_OUTPUT_CHARACTERS")) continue;
    if (pair("--max-expert-calls", "CASCADE_MAX_EXPERT_CALLS")) continue;
    if (pair("--max-expert-cost-usd", "CASCADE_MAX_EXPERT_COST_USD")) continue;
    if (pair("--max-session-cost-usd", "CASCADE_MAX_SESSION_COST_USD")) continue;
    if (pair("--classification", "CASCADE_CLASSIFICATION")) continue;
    if (pair("--harness-mode", "CASCADE_HARNESS_MODE")) continue;
    if (pair("--workspace-python", "CASCADE_WORKSPACE_PYTHON")) continue;
    if (pair("--workspace-sandbox-command", "CASCADE_WORKSPACE_SANDBOX_COMMAND")) continue;
    if (pair("--workspace-state-path", "CASCADE_WORKSPACE_STATE_PATH")) continue;
    if (pair("--pi-bin", "CASCADE_PI_BIN")) continue;
    if (arg === "--allow-contributor") { env.CASCADE_ALLOW_CONTRIBUTOR = "1"; index += 1; continue; }
    if (arg === "--deny-contributor") { env.CASCADE_ALLOW_CONTRIBUTOR = "0"; index += 1; continue; }
    if (arg === "--auto-consult") { env.CASCADE_AUTO_CONSULT = "1"; index += 1; continue; }
    if (arg === "--no-auto-consult") { env.CASCADE_AUTO_CONSULT = "0"; index += 1; continue; }
    if (arg === "--workspace") { env.CASCADE_WORKSPACE = "1"; index += 1; continue; }
    if (arg === "--no-workspace") { env.CASCADE_WORKSPACE = "0"; index += 1; continue; }
    if (arg === "--workspace-unsandboxed") { env.CASCADE_WORKSPACE_UNSANDBOXED = "1"; index += 1; continue; }
    if (arg === "--single") { env.CASCADE_MODE = "single"; index += 1; continue; }
    if (arg === "--dual") { env.CASCADE_MODE = "dual"; index += 1; continue; }
    passthrough.push(arg);
    index += 1;
  }
...
function isProjectApproved(args) {
  return args.includes("--approve") || args.includes("-a") || ["1", "true", "yes"].includes(String(process.env.CASCADE_PROJECT_TRUSTED || "").toLowerCase());
}

function effectiveConfig({ cwd, parsed, passthrough, throwOnError = true }) {
...
async function runPi(argv) {
  ...
  let startupWarning = "";
  try {
    loaded = effectiveConfig({ cwd, parsed, passthrough: parsed.passthrough, throwOnError: false });
    if (loaded.validation.errors.length) {
      startupWarning = `Cascade configuration was ignored: ${loaded.validation.errors.join("; ")}`;
      loaded = { ...loaded, config: structuredClone(DEFAULT_CONFIG) };
    }
  } catch (error) {
    const env = { ...process.env, ...parsed.env };
    const projectTrusted = isProjectApproved(parsed.passthrough);
    if (projectTrusted) env.CASCADE_PROJECT_TRUSTED = "1";
    loaded = { config: DEFAULT_CONFIG, env, projectTrusted };
    startupWarning = `Cascade configuration could not be loaded: ${error instanceof Error ? error.message : String(error)}`;
  }
  ...
  // Cascade must start like Pi. Model selection, authentication, privacy checks,
  // and role activation happen inside the running TUI. Never reject startup
  // merely because an optional worker/expert profile is unavailable.
  const piArgs = ["--extension", EXTENSION_PATH, ...parsed.passthrough];
  const childEnv = {
    ...env,
...
function initCommand(args) {
  ...
  console.log(`Created ${global ? "global" : "project"} Cascade configuration: ${path}`);
...
function configCommand(args) {
  ...
  if (action !== "show") throw new Error("Usage: cascade config show|path|example");
...
async function harnessCommand(args) {
  ...
  if (action === "replay") {
    if (!replayManifest) throw new Error("Usage: cascade harness replay CANDIDATE_ID MANIFEST [--output FILE] [--expert-reviewed] [--no-isolation]");
  ...
  throw new Error("Usage: cascade harness list|candidates|evaluate|replay|promote|rollback");
}


async function probeCommand(args) {
  const [role = "worker", ...rest] = args;
  if (!["worker", "expert"].includes(role)) throw new Error("Usage: cascade probe worker|expert [cascade options]");
  const loaded = loadForCommand(rest);
...
async function evalCommand(args) {
  ...
  if (!manifestPath) throw new Error("Usage: cascade eval MANIFEST [--output FILE]");
...
function compactionCommand(args) {
  ...
  if (action === "show") {
    console.log(JSON.stringify(getPiGlobalCompaction(), null, 2));
    return;
  }
  if (action !== "set") {
    throw new Error("Usage: cascade compaction show|set [--enabled true|false] [--reserve-tokens N] [--keep-recent-tokens N]");
  }
  const current = getPiGlobalCompaction();
  const next = { ...current.compaction };
  const enabled = parseBooleanFlag(flags.enabled, "--enabled");
  if (enabled !== undefined) next.enabled = enabled;
  if (flags["reserve-tokens"] !== undefined) next.reserveTokens = Number(flags["reserve-tokens"]);
  if (flags["keep-recent-tokens"] !== undefined) next.keepRecentTokens = Number(flags["keep-recent-tokens"]);
  const result = writePiGlobalCompaction(next);
  console.log(JSON.stringify(result, null, 2));
  console.log("Global compaction limits saved. They apply to new Cascade/Pi sessions.");
}

function updateCommand(args) {
  const flags = parseCommandFlags(args);
  const result = runSelfUpdate({ dryRun: Boolean(flags["dry-run"]) });
  if (result.dryRun) {
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  console.log("Cascade updated from GitHub. Start a new terminal session or run `rehash`, then check `cascade --version`.");
}
...
async function main() {
  ...
  if (["-v", "--version", "version"].includes(command)) {
    const runtime = runtimeSummary({ piBinary: "auto" });
    console.log(`cascade ${PACKAGE_VERSION}`);
  ...
  if (command === "update" || command === "pull") return updateCommand(argv.slice(1));
...
main().catch((error) => {
  console.error(`cascade: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
